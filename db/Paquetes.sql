...
--PRUEBAS TRABAJOS
CREATE OR REPLACE 
PACKAGE PRUEBAS_TRABAJOS AS
    
    PROCEDURE inicializar;
    PROCEDURE insertar (nombre_prueba VARCHAR2, w_fecha_fin trabajos.fecha_fin%TYPE, salidaEsperada BOOLEAN);
    PROCEDURE actualizar (nombre_prueba VARCHAR2, w_oid_t trabajos.oid_t%TYPE, w_fecha_fin trabajos.fecha_fin%TYPE, salidaEsperada BOOLEAN);
    PROCEDURE eliminar (nombre_prueba VARCHAR2, w_oid_t trabajos.oid_t%TYPE, salidaEsperada BOOLEAN);
    
END PRUEBAS_TRABAJOS;
/

CREATE OR REPLACE 
PACKAGE BODY PRUEBAS_TRABAJOS AS
    ...
    PROCEDURE insertar (nombre_prueba VARCHAR2, w_fecha_fin trabajos.fecha_fin%TYPE, salidaEsperada BOOLEAN) AS
        salida BOOLEAN := true;
        trabajo trabajos%ROWTYPE;
        w_oid_t trabajos.oid_t%TYPE;
    BEGIN
    
        INSERT INTO trabajos (oid_t, fecha_fin) VALUES (SEC_trabajos.nextval, w_fecha_fin);

        w_oid_t := SEC_trabajos.currval;
        SELECT * INTO trabajo FROM trabajos WHERE oid_t = w_oid_t;
        IF (trabajo.fecha_fin <> w_fecha_fin) THEN
            salida := false;
        END IF;
        COMMIT WORK;
    ...
    PROCEDURE actualizar (nombre_prueba VARCHAR2, w_oid_t trabajos.oid_t%TYPE, w_fecha_fin trabajos.fecha_fin%TYPE,
                          salidaEsperada BOOLEAN) AS
        salida BOOLEAN := true;
        trabajo trabajos%ROWTYPE;
    BEGIN
        
        UPDATE trabajos SET fecha_fin = w_fecha_fin WHERE oid_t = w_oid_t;
    ...
    PROCEDURE eliminar (nombre_prueba VARCHAR2, w_oid_t trabajos.oid_t%TYPE, salidaEsperada BOOLEAN) AS
        salida BOOLEAN := true;
        n_trabajos INTEGER;
    BEGIN
        ...
        SELECT COUNT(*) INTO n_trabajos FROM trabajos WHERE oid_t = w_oid_t;
        IF (n_trabajos <> 0) THEN
            salida := false;
        END IF;
        COMMIT WORK;
